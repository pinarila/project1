...
def delete_product(product_id):
    ...
    # Menghapus produk dari tabel demands berdasarkan ID
    cursor.execute('DELETE FROM demands WHERE id=%s', (product_id,))
    conn.commit()
    conn.close()

    # Memberikan notifikasi bahwa produk berhasil dihapus
...
def list_supply():
    ...
    if request.method == 'POST':
        ...
        # Generate a supply ID if not provided
...
def get_products(category_id):
    ...
    # Query products from the supply table based on category
...
def update_supply(supply_id):
    ...
    cursor.execute('UPDATE supply SET name=%s, category=%s, unit=%s, quantity=%s WHERE id=%s',
                   (supply_name, supply_category, supply_unit, supply_quantity, supply_id))
    conn.commit()
    conn.close()
...
@app.route('/delete_supply/<supply_id>')
...
# Route to redirect to supply page
@app.route('/supply')
def supply():
